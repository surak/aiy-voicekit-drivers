 ...
 */
 ...
 static int snd_rpi_aiy_voicebonnet_init(struct snd_soc_pcm_runtime *rtd)
 {
	 ...
	 printk(KERN_EMERG "snd_rpi_aiy_voicebonnet_init: Start Initialize.\n");
 
	 ret = snd_soc_dai_set_sysclk(codec_dai, RT5645_SCLK_S_MCLK,
					  PLATFORM_CLOCK, SND_SOC_CLOCK_IN);
	 if (ret < 0) {
		 printk(KERN_EMERG "snd_rpi_aiy_voicebonnet_init: failed to set sysclk\n");
 ...
 #endif
	 if (ret < 0) {
		 printk(KERN_INFO "snd_rpi_aiy_voicebonnet_init: failed to add headphone jack\n");
		 dev_err(rtd->card->dev, "can't add headphone jack: %d\n", ret);
		 return ret;
	 }
 
	 {
		 int result = rt5645_set_jack_detect(codec_dai->component, &headset_jack, NULL, NULL);
		 printk(KERN_EMERG "jack_detect: result=%d\n", result);
		 return result;
	 }
 }
 
 static int snd_rpi_aiy_voicebonnet_hw_params(struct snd_pcm_substream *substream,
						   struct snd_pcm_hw_params *params)
 {
	 ...
	 ret = snd_soc_dai_set_pll(codec_dai, 0, RT5645_PLL1_S_MCLK,
				   PLATFORM_CLOCK, freq);
	 if (ret < 0) {
		 dev_err(rtd->card->dev, "can't set codec pll: %d\n", ret);
		 return ret;
	 }
	 ...
	 ret = snd_soc_dai_set_sysclk(codec_dai, RT5645_SCLK_S_PLL1, freq,
					  SND_SOC_CLOCK_IN);
	 if (ret < 0) {
		 dev_err(rtd->card->dev, "can't set codec sysclk in: %d\n", ret);
		 return ret;
	 }
	 ...
	 ret = snd_soc_dai_set_sysclk(codec_dai, RT5645_SCLK_S_PLL1, freq,
					  SND_SOC_CLOCK_OUT);
	 if (ret < 0) {
		 dev_err(rtd->card->dev, "can't set codec sysclk out: %d\n", ret);
		 return ret;
	 }
 
	 printk(KERN_EMERG "snd_rpi_aiy_voicebonnet_hw_params: success\n");
 ...
 static int snd_rpi_aiy_voicebonnet_probe(struct platform_device *pdev)
 {
	 ...
	 printk(KERN_EMERG "snd_rpi_aiy_voicebonnet_probe: Start probe voicebonnet.\n");
 
	 if (dev->of_node) {
		 /* codecs は配列になっているので最初の要素にアクセス */
		 dai->codecs[0].of_node = of_parse_phandle(dev->of_node,
												 "aiy-voicebonnet,audio-codec", 0);
		 if (!dai->codecs[0].of_node) {
			 printk(KERN_EMERG "snd_rpi_aiy_voicebonnet_probe: codec parse error.\n");
		 ...
		 if (i2s_node) {
			 printk(KERN_EMERG "snd_rpi_aiy_voicebonnet_probe: Found I2S node.\n");
	 ...
	 if (ret < 0) {
		 printk(KERN_EMERG "snd_rpi_aiy_voicebonnet_probe: Failed to parse card name.\n");
	 ...
	 if (ret < 0) {
		 printk(KERN_EMERG "snd_rpi_aiy_voicebonnet_probe: Failed to register sound card.\n");
