 ...
 */
 ...
 #include <linux/gpio/driver.h>   /* 追加: gpio_chip 関連の宣言用 */
 #include <linux/i2c.h>
 ...
 {
	 /* gpiochip_get_data() は <linux/gpio/driver.h> で定義されています */
 ...
 static int aiy_gpio_request(struct gpio_chip *chip, unsigned int offset)
 {
	 ...
	 if (err < 0)
		 dev_err(chip->parent, "Request GPIO #%d failed: %d", offset, err);
	 return err;
 }
 ...
 static int aiy_gpio_get(struct gpio_chip *chip, unsigned int offset)
 {
	 ...
	 err = regmap_bulk_read(aiy->regmap, AIY_REG_GPIO_INPUT_LEVEL, bytes, sizeof(bytes));
	 if (err < 0) {
 ...
 static int aiy_gpio_set_impl(struct gpio_chip *chip, unsigned int offset, int value)
 {
	 struct aiy_io_i2c *aiy = to_aiy(chip);
										...
										unsigned int offset, int value)
 {
	 ...
	 err = regmap_write(aiy->regmap, AIY_REG_GPIO_BASE_MODE + offset, AIY_GPIO_MODE_OUTPUT);
	 if (err < 0) {
		 dev_err(chip->parent, "Set GPIO #%d as output failed: %d\n", offset, err);
		 return err;
	 }
 ...
 static int aiy_gpio_probe(struct platform_device *pdev)
 {
	 ...
	 /* chip->names は Kernel 6.6 では廃止されているため削除 */
	 chip->can_sleep = true;
