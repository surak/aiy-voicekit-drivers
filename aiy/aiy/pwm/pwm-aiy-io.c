 ...
 */
 ...
 /* pwm_chip から親の aiy_io_i2c 構造体を取得 */
						...
						uint8_t *prescaler_index)
 {
	 ...
	 unsigned long long scaled_period_cycles;
	 unsigned long long duty_cycles = duty_ns;
	 unsigned long long scaled_duty_cycles;
	 int i;
		 ...
		 if (scaled_period_cycles < AIY_PWM_MAX_CAP) {
			 *prescaler_index = i;
			 if (scaled_period_cycles == 0) {
				 dev_warn(&chip->dev, "Selected PWM Period too small.\n");
				 return -EINVAL;
			 }
			 ...
			 *duty_cyc = (uint16_t)scaled_duty_cycles;
			 dev_dbg(&chip->dev,
				 "Prescaler selected %d, period selected %llu, duty cycle selected %llu\n",
				 prescaler, scaled_period_cycles, scaled_duty_cycles);
			 break;
		 }
	 }
	 return 0;
					...
					int duty_ns, int period_ns)
 {
	 ...
	 err = aiy_calculate_8bit_settings(chip, duty_ns, period_ns,
					   &period_value, &duty_value, &prescaler_value);
	 if (err != 0) {
		 dev_err(&chip->dev, "Failed to calculate PWM settings.");
		 return -EINVAL;
	 }
	 err = aiy_pwm_write_setting(chip, pwm->hwpwm, prescaler_value, duty_value, period_value);
	 if (err != 0) {
 ...
 /* 
  * 新たに pwm_state を使用する apply 関数を実装
  */
 static int aiy_pwm_apply(struct pwm_chip *chip, struct pwm_device *pwm,
			   const struct pwm_state *state)
 {
	 int ret;
	 ret = aiy_pwm_config(chip, pwm, state->duty_cycle, state->period);
	 if (ret < 0)
		 return ret;
	 if (state->enabled)
		 return aiy_pwm_enable(chip, pwm);
	 else {
		 aiy_pwm_disable(chip, pwm);
		 return 0;
	 }
 }
 ...
 static int aiy_pwm_enable(struct pwm_chip *chip, struct pwm_device *pwm)
 {
	 ...
	 err = regmap_write(aiy->regmap, AIY_REG_GPIO_BASE_MODE + pin_offset, selected_mode);
	 if (err != 0)
