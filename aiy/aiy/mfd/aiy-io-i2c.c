...
/*
 * __i2c_transfer の代わりに公開APIの i2c_transfer を使用する。
 */
				      ...
				      const char *buf, size_t count)
{
	...
	while (remaining_size > 0) {
		...
		dev_info(dev, "Firmware page: offset=%d, size=%d", page_offset,
			 page_size);
		err = aiy_io_mcu_write_direct(i2c, MCU_BOOTLOADER_ADDR, &packet,
					    sizeof(packet));
		if (err < 0) {
...
/*
 * probe 関数のシグネチャを i2c_client のみとします。
 */
static int aiy_io_i2c_probe(struct i2c_client *i2c)
{
	...
	/* devm を用いた MFD サブデバイスの登録 */
	err = devm_mfd_add_devices(dev, PLATFORM_DEVID_NONE, aiy_io_devs,
...
static void aiy_io_i2c_remove(struct i2c_client *i2c)
{
	...
	/* MFD サブデバイスおよび sysfs ノードは devm により自動解放 */
